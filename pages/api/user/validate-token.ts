...
async function checkJWT(req: NextApiRequest, res: NextApiResponse<Data>) {
    ...
    try {
        ...
        return; //Para que no siga ejecutando el codigo si el token es invalido
    }

    //Verificamos si el token es valido
    if (!mongoose.Types.ObjectId.isValid(userID)) { return res.status(401).json({ message: 'Invalid token' }); }
